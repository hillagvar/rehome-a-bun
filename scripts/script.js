...
for (i = 0; i < bunnyNames.length; i++) {
    bunnyNames[i].addEventListener('click', function(event) {
        ...
        modal.querySelector('.modal-link').setAttribute('href', `view.php?id=${bunny.id}`);
    })
}
...
for (i = 0; i < abilities.length; i++) {
    abilities[i].addEventListener('click', function(event) {
        ...
        modal.querySelector('.ability-description').innerText = `${ability.description}`;
        
    })
}

if (document.querySelector('.modal-wrapper-ability') !== null) {
    document.querySelector('.modal-wrapper-ability').addEventListener('click', function(event) {
        event.target.classList.remove('active');
      });
}
